# SPDX-FileCopyrightText: Copyright © contributors to the Not-CloudNativePG project.
# SPDX-License-Identifier: Apache-2.0
metadata = {
  name                     = "pg-rrule"
  sql_name                 = "pg_rrule"
  image_name               = "pg-rrule"
  licenses                 = ["MIT"]
  shared_preload_libraries = []
  postgresql_parameters    = {}
  extension_control_path   = []
  dynamic_library_path     = []
  ld_library_path          = ["system"]
  bin_path                 = []
  env                      = {}
  auto_update_os_libs      = true
  required_extensions      = []
  create_extension         = true

  versions = {
    bookworm = {
      "18" = {
        // renovate: suite=bookworm-pgdg depName=postgresql-18-pg-rrule
        package = "0.3.0-1.pgdg12+1"
        // renovate: suite=bookworm-pgdg depName=postgresql-18-pg-rrule extractVersion=^(?<version>\d+\.\d+\.\d+)
        sql     = "0.3.0"
      }
    }
    trixie = {
      "18" = {
        // renovate: suite=trixie-pgdg depName=postgresql-18-pg-rrule
        package = "0.3.0-1.pgdg13+1"
        // renovate: suite=trixie-pgdg depName=postgresql-18-pg-rrule extractVersion=^(?<version>\d+\.\d+\.\d+)
        sql     = "0.3.0"
      }
    }
  }
}
